		...
		methods: {
					...
					await getAllHistory().then((val: any) => {
						...
						if (val) {
							val.forEach((item: any) => {
								const index1 = this._RG.config.data.rewardId.everyLogin.indexOf(item.rewardId);
								const index3 = this._RG.config.data.rewardId.rotate.indexOf(item.rewardId);
								const index2 = this._RG.config.data.rewardId.weekpay.indexOf(item.rewardId);
								// console.log(index1, index2, index3);
								index1 !== -1 && (this.act1Infos[index1].isDisabled = true);
								index2 !== -1 && (this.act2Infos[index2].isDisabled = true);
								index3 !== -1 &&
									this.act3GiftsCounts.splice(index3, 1, this.act3GiftsCounts[index3] + 1);
							});
						}
					});
			...
			async searchRewardBtn() {
				if (isLogin()) {
					...
					if (data) {
						if (!data.length) {
							return this.$dialog.show('tip', window._RG.config.tip.giftArr_null);
						}
